...
void Human::init(float health, float speed, glm::vec2 position)
{
	...
	m_health = health;
	m_textureID = GameEngine2D::ResourceManager::getTexture("Textures/human.png").id;
}

void Human::update(const vector<string>& levelData, vector<Human*>& humans, vector<Zombie*>& zombies, float deltaTime)
{
	static mt19937 randomEngine;
