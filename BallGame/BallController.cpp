...
void BallController::updateBalls(std::vector <Ball>& balls, SpatialGrid* spatialGrid, float deltaTime, int maxX, int maxY) {
    ...
    for (size_t i = 0; i < balls.size(); i++) {
		...
		Cell* newCell = spatialGrid->getCell(ball.position);
		if (newCell != ball.ownerCell) {
			//Need to shift the ball
			spatialGrid->removeBallFromCell(&balls[i]);
			spatialGrid->addBall(&balls[i], newCell);
		}
        
    }

	//Update collision with spatial partitioning
	updateCollision(spatialGrid);
...
void BallController::updateCollision(SpatialGrid* spatialGrid)
{
	for (int i = 0; i < spatialGrid->m_cells.size(); i++)
	{
		int x = i % spatialGrid->m_numXCells;
		int y = i / spatialGrid->m_numXCells;

		Cell& cell = spatialGrid->m_cells[i];

		//Loop through all balls in a cell
		for (int j = 0; j < cell.balls.size(); j++)
		{
			//Check collision for balls own cell
			/*
			XX     XX
			XX  => XO
			X      X
			*/
			...
			if (x > 0)
			{
				...
				*/
				...
				///Left
				checkCollision(ball, spatialGrid->getCell(x - 1, y)->balls, 0);

				if (y > 0)
				{
					/// Bottom Left
					checkCollision(ball, spatialGrid->getCell(x - 1, y - 1)->balls, 0);
				}

				if (y < spatialGrid->m_numYCells - 1)
				{
					/// Top Left
					checkCollision(ball, spatialGrid->getCell(x - 1, y + 1)->balls, 0);
				}
			...
			{
				///Bottom cell
				checkCollision(ball, spatialGrid->getCell(x, y - 1)->balls, 0);
			}
		}
	}
}
...
void BallController::checkCollision(Ball* ball, std::vector<Ball*>& ballsToCheck, int startingIndex)
{
	...
	{
		checkCollision(*ball, /*The value pointed to by ballsToCheck[i] because we need to pass a reference*/*ballsToCheck[i]);
	}
}
